...
CREATE TABLE Alunos (     -- Tudo certo
...
CREATE TABLE Cursos (     -- Alter table para adicionar carga horaria  e alter table para o nome de curso, estava poucos varchar
...
CREATE TABLE Disciplinas (             -- remover o nome do professor, adicionar o idCurso
...
FOREIGN KEY (turma_id) REFERENCES Turmas(turmaId),
FOREIGN KEY (disciplinaId) REFERENCES Disciplinas(disciplinaId)
);

-- FASE 02 ADICIONAR AS TABELAS QUE PRECISAMOSSS

CREATE TABLE Professores (                -- CRIOU ESSA NOVA TABELA
  professorId INT NOT NULL PRIMARY KEY AUTO_INCREMENT, 
  nome VARCHAR(40) NOT NULL, 
  email VARCHAR(50) NOT NULL
);

CREATE TABLE AlunosNosCursos (                -- CRIAÇÃO DE NOVA TABELA TBM
  id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, 
...
CREATE TABLE Notas (            -- CRIOU NA FASE 2
...
CREATE TABLE DisciplinasCursadas (  -- CRIOU NA FASE 2
  id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, 
  aluno_cpf CHAR(11) NOT NULL, 
  disciplinaId INT NOT NULL, 
  FOREIGN KEY (aluno_cpf) REFERENCES Alunos(cpf), 
...
ALTER TABLE Disciplinas DROP COLUMN professor;    -- REMOVER P PODER FAZER A LIGAÇÃO COM O IDPROFESSOR


ALTER TABLE Disciplinas                                  -- AGORA CURSOS TEM LIGAÇÕES COM DISCIPLINAS
ADD COLUMN cursoid INT NOT NULL,
ADD CONSTRAINT fk_cursoid FOREIGN KEY (cursoid) REFERENCES Cursos(cursoid);


ALTER TABLE Disciplinas                                              -- FAZENDO A REFERENCIA DE PROFESSOR
ADD COLUMN professorId INT NOT NULL,
ADD CONSTRAINT fk_professorId FOREIGN KEY (professorId) REFERENCES Professores(professorId);


ALTER TABLE Cursos ADD COLUMN cargaHoraria INT NOT NULL;            -- ADICIONEI UMA CARGA HORARIA PARA OS CURSOS


ALTER TABLE Cursos MODIFY COLUMN nome VARCHAR(50) NOT NULL;       -- Mudei a quantidade de Varchar
...
-- DisciplinaNaTurma 
-- Turma 1
INSERT INTO DisciplinasNasTurmas (turma_id, disciplinaId) VALUES
...
-- Continua inserindo os demais alunos nas cinco primeiras turmas
INSERT INTO AlunosNasTurmas (aluno_cpf, turmaId) VALUES
('12345678921', 1),
('12345678921', 2),
('12345678921', 3),
...
-- QUESTÃO 7                    
SELECT 
    SUM(d.cargaHoraria) AS totalHoras FROM DisciplinasCursadas dc
JOIN Disciplinas d ON dc.disciplinaId = d.disciplinaId;

-- QUESTÃO 7, SE A QUANTIDADE FOR POR ALUNO/CPF QUE É A CHAVE PRIMARIA DE ALUNOS
SELECT dc.aluno_cpf, SUM(d.cargaHoraria) AS totalHoras FROM DisciplinasCursadas dc 
JOIN Disciplinas d ON dc.disciplinaId = d.disciplinaId GROUP BY dc.aluno_cpf;

-- QUESTÃO 8
SELECT AVG(nota) AS mediaNotas FROM Notas;

-- QUESTÃO 9  Liste os nomes e as disciplinas de cada turma
...
JOIN Turmas ON DisciplinasNasTurmas.turma_id = Turmas.turmaId
...
JOIN AlunosNasTurmas ON DisciplinasNasTurmas.turma_id = AlunosNasTurmas.turmaId
JOIN Alunos ON AlunosNasTurmas.aluno_cpf = Alunos.cpf
